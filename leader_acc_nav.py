...
FORMATION_SPACING = 130.0
PATH_RECORD_DIST = 4.0

COLLISION_SAFE_DIST = 110.0
COLLISION_STOP_DIST = 70.0
...
FORMATIONS = {
    ...
    "line": {
        ...
        2: {"side_offset": 0.0, "fb_offset": 0.0, "target_dist": 2.0 * FORMATION_SPACING},
        3: {"side_offset": 0.0, "fb_offset": 0.0, "target_dist": 3.0 * FORMATION_SPACING},
    }
}
    ...
    try:
        while True:
            ...
            for rid in [1, 2, 3]:
                ...
                if not paused and robot_states[rid]["found"] and target_active:
                    ...
                    if len(path) > 0:
                        ...
                        # Çarpışmaları önlemek için mesafe yeterli olana kadar motorları beklemede tut
                        if current_path_len < effective_target_dist:
                            f_left_cmd, f_right_cmd = 0, 0
                        else:
                        ...
                        # Collision avoidance (ACC) scaling based on priority queue (0 -> 1 -> 2 -> 3)
            ...
            if key == ord("q") or key == 27:
                ...
                FORMATION_SPACING = max(60.0, FORMATION_SPACING - 5.0)
                FORMATIONS["triangle"][1]["side_offset"] = FORMATION_SPACING
                FORMATIONS["triangle"][3]["side_offset"] = -FORMATION_SPACING
                FORMATIONS["triangle"][1]["target_dist"] = FORMATION_SPACING
                FORMATIONS["triangle"][2]["target_dist"] = FORMATION_SPACING
                FORMATIONS["triangle"][3]["target_dist"] = FORMATION_SPACING
                FORMATIONS["line"][1]["target_dist"] = FORMATION_SPACING
                FORMATIONS["line"][2]["target_dist"] = 2.0 * FORMATION_SPACING
                FORMATIONS["line"][3]["target_dist"] = 3.0 * FORMATION_SPACING
                print(f"[FORMASYON ARALIGI] {FORMATION_SPACING}px")
            elif key == ord("p") or key == ord("P"):
                FORMATION_SPACING = min(200.0, FORMATION_SPACING + 5.0)
                FORMATIONS["triangle"][1]["side_offset"] = FORMATION_SPACING
                FORMATIONS["triangle"][3]["side_offset"] = -FORMATION_SPACING
                FORMATIONS["triangle"][1]["target_dist"] = FORMATION_SPACING
                FORMATIONS["triangle"][2]["target_dist"] = FORMATION_SPACING
